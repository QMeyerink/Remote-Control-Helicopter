...
void pid_update(int32_t altitude, int32_t altitude_setpoint, int32_t yaw, int32_t yaw_setpoint, double delta_t)
{
    ...
    error_inter_main += altitude_error;
    error_inter_tail += yaw_error;
    ...
    altitude_control = 10 + (altitude_error*M_Kp) + (error_inter_main*M_Ki) + (altitude_error_deriv*M_Kd);
    yaw_control = (yaw_error*T_Kp) + (error_inter_main*T_Ki) + (yaw_error_deriv*T_Kd);
